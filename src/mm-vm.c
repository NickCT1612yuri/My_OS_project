...
//  int incnumpage =  inc_amt / PAGING_PAGESZ;
  ...
  /* Keep remainder of expanded VMA in free-region list for future first-fit allocs. */
  if (newrg->rg_end < cur_vma->vm_end)
  {
    struct vm_rg_struct *freerg = init_vm_rg(newrg->rg_end, cur_vma->vm_end);
    enlist_vm_rg_node(&cur_vma->vm_freerg_list, freerg);
  }

  /* sbrk tracks the mapped VMA end; allocations after this come from free list. */
